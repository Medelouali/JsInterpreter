...
std::string symetry(const std::string& cmd){
	...
	for(int i=0; i<3; i++)if(countChar(cmd, vect[i])%2)return "You forgot to open or close " +toStr(vect[i]);
	for(int i=3; i<vect.size(); i++)
		if(paren(cmd, vect[i])){
			err="You forgot to open or close " + toStr(vect[i]);
			break;
		};
	return err;
};
...
std::string syntax(std::string str){
	...
	for(long unsigned int i=0; i<newStr.length()-1; i++){
		...
		if(i>0 && index==-1){
			err=toStr(newStr[i])+ " is not allowed in Enigma commands as input, please read the docs";
			break;
		};
		...
		if(index_1!=-1 && index_2!=-1 && errorMatrix[index_1][index_2]=='0'){
			if(i==0)
				err=toStr(newStr[i+1]) +
					" can't be used in the beginning of a command, please read the docs";
			else if(i==newStr.length()-2)
				err=toStr(newStr[i]) + " can't be used at the end of a command, please read the docs";
			else{
				err="The following characters can't be used in a sequence: "+ toStr(newStr[i])+
